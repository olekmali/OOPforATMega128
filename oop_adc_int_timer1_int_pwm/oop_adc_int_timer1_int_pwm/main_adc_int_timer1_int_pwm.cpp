...
public:
    virtual void    handle();
    myInterrupt(uint16_t ADC_interval, PWM_Generator<uint8_t>* GEN, LEDs* PWM_Pins)
        : counter(0), interval(ADC_interval), PWM_GEN(GEN), PWM_PIN(PWM_Pins) {}    
private:
    uint16_t                counter;
    uint16_t                interval;
    PWM_Generator<uint8_t>* PWM_GEN;
    LEDs*                   PWM_PIN;
};
...
int main(void)
{
    ...
    PWM_Generator<uint8_t> PWM(2, 100, 100);
    ...
    while(1)
    {
        ...
        PWM.setPWM(1, static_cast<uint16_t>(100)*V1.getRecent(1)/255);
