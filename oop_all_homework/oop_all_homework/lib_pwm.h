...
template<typename pwmcnt_t>
class PWM_Generator {
public:
    PWM_Generator(/* TBA: PortSelection, */ uint8_t channels = 8, pwmcnt_t resolution = 100, pwmcnt_t percent100value = 100 );
...
private:
    static const uint8_t PWM_CH_MAX = 8;
    pwmcnt_t MyPWMCoef_params[PWM_CH_MAX];
    pwmcnt_t MyPWMCoef_reload[PWM_CH_MAX];
...
template<typename pwmcnt_t>
PWM_Generator<pwmcnt_t>::PWM_Generator(/* TBA: PortSelection, */ uint8_t channels, pwmcnt_t resolution, pwmcnt_t percent100value )
:   MyPWMCoef_change(false), PWM_RESOLUTION(resolution), PWM_100_PERCENT(percent100value), PWM_CH_CNT(channels), PWM_CH_MASK(0), counter(0)
{
    for(uint8_t i=0; i<PWM_CH_CNT; ++i)
    {
...
template<typename pwmcnt_t>
void PWM_Generator<pwmcnt_t>::setPWM(uint8_t channel, pwmcnt_t percentage)
{
...
template<typename pwmcnt_t>
pwmcnt_t PWM_Generator<pwmcnt_t>::getPWM(uint8_t channel) const
{
    pwmcnt_t value;
    uint8_t sreg_save = SREG;           // Preserve Current Interrupt Status
    cli();
...
template<typename pwmcnt_t>
uint8_t PWM_Generator<pwmcnt_t>::NextStepUpdate(uint8_t pwmbyte)
{
